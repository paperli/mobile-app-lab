// Offline ElevenLabs asset generation. No credentials enter the browser bundle.
import { writeFile } from 'node:fs/promises';
import { PAIRING_SUCCESS_PROMPT } from '../src/vo.js';
...
const response = await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${encodeURIComponent(voice)}?output_format=mp3_44100_128`, {
  ...
  body:JSON.stringify({text:PAIRING_SUCCESS_PROMPT,model_id:'eleven_multilingual_v2',voice_settings:{stability:0.5,similarity_boost:0.75,use_speaker_boost:true}}),
});
if (!response.ok) {
...
await writeFile(new URL('../public/assets/host/10-pairing-success.mp3',import.meta.url),audio);
console.log(`Saved ElevenLabs pairing-success prompt (${audio.length} bytes).`);
